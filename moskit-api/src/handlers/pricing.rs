...
pub async fn get_global_pricing(State(state): State<Arc<AppState>>) -> ApiResult<GlobalPricing> {
    ...
    let components = vec![
        ...
        PricingItem { id: "handle_metal".to_string(), name: "Ручка металл (пара)".to_string(), price: 50.0 },
        PricingItem { id: "mount_plastic".to_string(), name: "Крепления пластик (комплект)".to_string(), price: 40.0 },
        PricingItem { id: "mount_metal".to_string(), name: "Крепления металл (комплект)".to_string(), price: 40.0 },
