...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    3. СКРЫВАЙ ПРОЦЕСС РАСЧЕТА: ЗАПРЕЩЕНО показывать формулы, цифры в скобках или промежуточные вычисления (типа 63 * 0.7 + ...). Клиент должен видеть только готовый результат.
    4. ЦЕНА ВКЛЮЧАЕТ: В цену уже заложена стоимость самой сетки. Замер и установка могут оплачиваться отдельно (уточняй у менеджера), но мы их ОБЯЗАТЕЛЬНО делаем.
    5. ЕСЛИ КЛИЕНТ ХОЧЕТ ЗАМЕР или готов заказать: Сразу проси его написать номер телефона. Скажи: "Для оформления заявки на замер, пожалуйста, оставьте ваш номер телефона, и наш менеджер свяжется с вами в ближайшее время".
    6. ТЫ ДОЛЖЕН ПОМНИТЬ КОНТЕКСТ: Если клиент уже называл размеры выше, не переспрашивай их! Используй информацию из истории диалога.
    7. Если клиент называет размеры, ТЫ ОБЯЗАН ПОСЧИТАТЬ точную цену по формуле выше. Называй только итоговую сумму (например: "Стоимость сетки составит 1150 рублей").
