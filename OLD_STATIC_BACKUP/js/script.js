function calc() {
  ...
  // Определяем индекс в массиве PRICES_STANDART
  ...
  // radio_value: 1-белая, 2-коричневая, 3-антрацит, 4-RAL
  var offset = (radio_value - 1) * 16;
  price = PRICES_STANDART[offset + index];

  // Отладочная информация
  console.log("=== ОТЛАДКА КАЛЬКУЛЯТОРА ===");
  console.log("Размеры:", $("#horizontal").val(), "x", $("#vertical").val());
  console.log("Площадь:", area_doors);
  console.log("radio_value:", radio_value);
  console.log("index:", index);
  console.log("offset:", offset);
  console.log("Индекс в массиве:", offset + index);
...
function setMinimalSliderValues() {
  // Всегда выбираем белую рамку по умолчанию
  var minPrice = PRICES_STANDART[0]; // белая рамка, индекс 0
  var minPriceIndex = 0;
  ...
  // Дополнительная проверка - убеждаемся, что значения установлены
  console.log("Установлены размеры:", $("#horizontal").val(), "x", $("#vertical").val());
  
  // Устанавливаем активный цвет рамки с минимальной ценой
  var colorIndex = (minPriceIndex / 16) + 1; // 1-белая, 2-коричневая, 3-антрацит, 4-RAL
  $("#block_radio .item").removeClass("active");
  $("#block_radio .item[data-radio='" + colorIndex + "']").addClass("active");
  $("#radio_value").val(colorIndex);
  
  console.log("Автоматически выбрана минимальная цена:", minPrice, "для цвета рамки с индексом", colorIndex);
}
