...
const faqSchema = computed(() => ({
  ...
  '@type': 'FAQPage',
  mainEntity: [
    { '@type': 'Question', name: `Чем Антимошка отличается от обычной сетки в ${tenant.config.city || 'Чебоксарах'}?`, acceptedAnswer: { '@type': 'Answer', text: `У Антимошки ячейка 0,8×0,8 мм — в 2 раза мельче обычной (1,2×1,2 мм). В ${tenant.config.city || 'Чебоксарах'} она задерживает мелких мошек, гнус и тополиный пух, которые проходят через стандартную сетку.` } },
    { '@type': 'Question', name: `Какая ячейка у сетки Антимошка от ${tenant.config.dealer_name || 'Сетки 21'}?`, acceptedAnswer: { '@type': 'Answer', text: 'Полотно Micro Mesh с размером ячейки 0,8×0,8 мм. Прозрачность около 50%, воздух проходит свободно.' } },
        ...
        <div class="flex flex-col lg:flex-row gap-12 items-stretch mb-8 min-h-[440px]">
          <div class="lg:w-1/2 flex flex-col justify-start pt-4 min-h-[440px]">
            <h1 class="text-4xl md:text-5xl font-black mb-6 leading-tight uppercase tracking-tight">
              Сетка <span class="text-brand-blue" :style="{ color: tenant.config.branding?.primary_color || '#2A6AB2' }">Антимошка</span> — от мошек и тополиного пуха
            </h1>
          ...
          <div class="lg:w-1/2 relative flex items-start pt-4 justify-end">
            <div class="relative w-full max-w-[640px]">
              <HeroImage
                ...
                :title="`Сетка Антимошка в ${tenant.config.city || 'Чебоксарах'}`"
