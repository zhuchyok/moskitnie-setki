        ...
        <div class="flex flex-col lg:flex-row gap-12 items-stretch mb-8 min-h-[440px]">
          <div class="lg:w-1/2 flex flex-col justify-center min-h-[440px]">
            <h1 class="text-4xl md:text-5xl font-black mb-6 leading-tight uppercase tracking-tight">
              Сетка <span class="text-brand-blue" :style="{ color: tenant.config.branding?.primary_color || '#2A6AB2' }">Ультравью</span> — более 13 лет опыта
            </h1>
            <div style="display:none" data-ai-summary>
              {{ tenant.config.dealer_name || 'Сетки 21' }}: Собственное производство москитных сеток Ультравью (Ultraview) в {{ tenant.config.city || 'Чебоксарах и Новочебоксарске' }}. Опыт более 13 лет.
            ...
            <p class="text-lg text-gray-600 mb-8 leading-relaxed font-medium text-justify">
              {{ tenant.config.seo?.content?.ultravyu || `Сетка Ультравью в ${tenant.config.city || 'Чебоксарах и Новочебоксарске'} от 1000 ₽. Собственное производство, работаем для вас уже более 13 лет. Изготовим по индивидуальным размерам за 2 дня. Повышенная прозрачность и мелкая ячейка — защита от комаров, мошек и тополиного пуха без затемнения окна от компании ${tenant.config.dealer_name || 'Сетки 21'}.` }}
            </p>
